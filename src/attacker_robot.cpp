...
class AttackerRobot : public Robot {
  public:
    class AttackerKicker : public Kicker {
      ...
      public:
        AttackerKicker(Robot* robot) : Kicker(robot) {
            ...
            dReal z_offset = -(rob->cfg->robotSettings.RobotHeight) * 0.5f + rob->cfg->robotSettings.WheelRadius -
            ...
            disc = new PCylinder(x, y, z + z_offset,
                                 ...
                                 rob->cfg->robotSettings.KickerHeight, rob->cfg->robotSettings.KickerMass * 0.99f, 0.9,
                                 0.9, 0.9, -1, false);
        ...
        virtual bool isTouchingBall() {
            ...
            if (dist < (disc->getRadius() + rob->cfg->BallRadius()) && bz < rob->cfg->robotSettings.KickerHeight) {
                return true;
            } else {
                return false;
            }
        ...
        //< NOTE: you only need to think about the case when an user change the robot manually, i.e. robot is ,parallel
        //to the ground.
    ...
    virtual void initialize(PWorld* world, PBall* ball, ConfigWidget* _cfg, dReal x, dReal y, dReal z, dReal r, dReal g,
                            dReal b, int rob_id, int wheeltexid, int dir) {
        m_r = r;
        m_g = g;
        m_b = b;
        w = world;
        ...
        dummy =
            new PBall(x, y, z, cfg->robotSettings.RobotCenterFromKicker, cfg->robotSettings.BodyMass * 0.01f, 0, 0, 0);
